...
function updateSelected(e) {
  var pos = getMousePosition(e);
  gBezierPath.updateSelected(snap(pos));
  render();
}
...
function LineSegment(pt, prev) {
  ...
  this.toJSString = function() {
    var r = 10.0;
    if (!my.prev)
      return '  shape.moveTo(' + Math.round(my.pt.x())/r + ', ' + Math.round(my.pt.y())/r + ');';
    else {
      var ctrlPt1x = 0;
      var ctrlPt1y = 0;
      var ctrlPt2x = 0;
      ...
      if (my.ctrlPt1) {
        ctrlPt1x = Math.round(my.ctrlPt1.x())/r;
        ctrlPt1y = Math.round(my.ctrlPt1.y())/r;
      }

      if (my.ctrlPt2) {
        ctrlPt2x = Math.round(my.ctrlPt2.x())/r;
        ctrlPt2y = Math.round(my.ctrlPt2.y())/r;
      }
      if (my.pt) {
        x = Math.round(my.pt.x())/r;
        y = Math.round(my.pt.y())/r;
      }

      return '  shape.bezierCurveTo(' + ctrlPt1x + ', ' +
              ctrlPt1y + ', ' +
              ctrlPt2x + ', ' +
              ctrlPt2y + ', ' +
              x + ', ' +
